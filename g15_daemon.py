#!/usr/bin/env python3
"""
Dell G15 Controller Daemon

Daemon seguro que executa como root para controlar hardware Dell G15.
Expõe API via Unix socket para clientes sem privilégios.

ATENÇÃO: Este daemon executa com privilégios root.
Todas operações são validadas e logadas por segurança.
"""
...
class G15HardwareController:
    """
    Controlador de hardware Dell G15 isolado e seguro.
    Apenas operações de leitura/escrita de hardware, sem GUI.
    """
    ...
    def __init__(self):
        ...
        # Caminhos dos sensores hwmon
        self.hwmon_path = None
        self.hwmon_fans = {}
        self.hwmon_temps = {}
        
        # Logger específico para hardware
    ...
    def _validate_security(self):
        """Validações críticas de segurança antes de iniciar"""
        # Verificar se está executando como root
        if os.geteuid() != 0:
            self.logger.critical("SECURITY: Daemon must run as root")
            sys.exit(1)
            
        # Verificar se paths críticos existem
    ...
    def _detect_hwmon_sensors(self):
        """Detectar sensores hwmon Dell de forma segura"""
            ...
            try:
                ...
                if os.path.exists(name_file):
                    ...
                    if hwmon_name in dell_hwmon_names:
                        ...
                        # Detectar sensores de ventoinha  
    ...
    def _read_hwmon_sensor(self, sensor_path: str) -> int:
        """Ler valor de sensor hwmon com validação"""
        try:
    ...
    def _acpi_call_real(self, wmi_code: str, args: list = None) -> str:
        """Chamada ACPI real com validação de segurança"""
        if args is None:
            args = []
    ...
    # Métodos de leitura de hardware (seguros)
    def get_cpu_temp(self) -> int:
        """Obter temperatura CPU via hwmon (preferencial) ou ACPI"""
        # Tentar hwmon primeiro
        ...
        return 45  # Valor padrão seguro

    def get_gpu_temp(self) -> int:
        """Obter temperatura GPU via hwmon (preferencial) ou ACPI"""
        # Tentar hwmon primeiro
    ...
    def get_fan_rpm(self, fan_id: int) -> int:
        """Obter RPM da ventoinha com validação"""
    ...
    def get_fan_boost(self, fan_id: int) -> int:
        """Obter boost da ventoinha com validação"""
        if not isinstance(fan_id, int) or fan_id not in [1, 2]:
            self.logger.error(f"SECURITY: Invalid fan_id: {fan_id}")
            return 0
    ...
    def get_g_mode_status(self) -> bool:
        """Verificar status do G-Mode"""
        result = self._acpi_call_real("0x25", ["0x02"])
        return result == "0x1"

    # Métodos de controle de hardware (críticos - com logging)
    def set_power_mode(self, mode: PowerMode) -> bool:
        """Definir modo de energia com validação rigorosa"""
    ...
    def set_fan_boost(self, fan_id: int, percentage: int) -> bool:
        """Definir boost da ventoinha com validação rigorosa"""
...
class G15DaemonServer:
    """
    Servidor daemon seguro que expõe API para controle do hardware.
    Executa como root, mas com validações rigorosas de segurança.
    """
    ...
    def setup_logging(self):
        """Configurar logging detalhado para auditoria"""
        logging.basicConfig(
            level=logging.INFO,
    ...
    def validate_request(self, client_addr: str, request_data: dict) -> bool:
        """Validar request com rate limiting"""
        # Rate limiting simples
        now = time.time()
        if client_addr not in self.client_requests:
            self.client_requests[client_addr] = []
        
        # Limpar requests antigos (> 10 segundos para rate limiting mais flexível)
        self.client_requests[client_addr] = [
            req_time for req_time in self.client_requests[client_addr]
            if now - req_time < 10
        ]
        
        # Máximo 50 requests por 10 segundos (300/minuto)
            ...
            if action == 'authenticate':
                ...
                return {
                    ...
                    "data": {
                        ...
                        "g_mode": self.hardware.g_mode_active  # Usa a variável de estado ao invés do hardware
                    }
                }
    ...
    def handle_client(self, client_socket, client_addr):
        """Lidar com cliente de forma segura"""
        try:
    ...
    def start_server(self):
        ...
        try:
            ...
            # Definir permissões do socket para que usuários possam conectar
            os.chmod(self.socket_path, 0o666)  # rw-rw-rw-
            
            # Alternativa mais segura: definir grupo como 'users' se disponível
            try:
                import grp
                users_gid = grp.getgrnam('users').gr_gid
                os.chown(self.socket_path, -1, users_gid)  # Manter owner, mudar grupo
                os.chmod(self.socket_path, 0o660)  # rw-rw----
                self.logger.info("Socket permissions set for 'users' group")
            except (KeyError, OSError):
                # Fallback para permissões mais abertas se grupo 'users' não existir
            ...
            while self.running:
                try:
                    ...
                    if self.running:  # Só log se não estivermos parando
